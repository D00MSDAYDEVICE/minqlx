# backfill.py - minqlx plugin by Doomsday - https://github.com/D00MSDAYDEVICE/minqlx
#
# Credits: this is BarelyMiSSeD's "bots" bot-filling plugin
# (https://github.com/BarelyMiSSeD), adapted for this server and renamed to
# backfill.py. All credit for the original bot-filling logic, the
# specqueue-integrated join queue, and the per-persona bot skill/model data
# below goes to BarelyMiSSeD.
#
# Changes in this version (2.41):
#   - Game-state mutations (.kick(), .model =, self.msg()/center_print())
#     that ran directly on a background @minqlx.thread are now queued
#     through @minqlx.next_frame instead - see _safe_kick_bot() and the
#     next_frame closures in kick_bot()/check_for_extra_bots()/
#     reset_players_models().
#   - Bots are re-resolved by client id (self.player(id)) right before
#     being kicked instead of kicking a possibly-stale captured Player
#     object.
#   - Delayed callbacks (@minqlx.delay(...)) and the bot-only-game Timer
#     now check self._alive, set by a new "unload" hook, so a plugin
#     reload can't leave a stale callback running against a dead instance.
#   - get_bot() no longer crashes if the bot name pool is ever exhausted;
#     add_bots() bails out of its fill loop cleanly instead.
#   - Added !setskill (see below).
#
# Changes in this version (2.42):
#   - Some server managers don't let admins control qlx_plugins load order,
#     so specqueue can end up listed there too even though backfill already
#     loads it internally (see the IMPORTANT note below). check_server()
#     now checks for that and unloads the redundant standalone copy - see
#     _reclaim_duplicate_specqueue().
#
# Changes in this version (2.43):
#   - The engine's own bot_nochat cvar doesn't reliably stop QL bots from
#     using say/say_team (this is a known engine-level limitation, not
#     something a cvar can fix from the plugin config). Added
#     qlx_backfillMuteBots (default "1"): backfill now intercepts and drops
#     chat/teamchat coming from bot players itself via a client_command
#     hook, before it's ever broadcast. Toggle at runtime with !mutebots.
#
...
# If your server manager doesn't let you control qlx_plugins (so specqueue ends up listed there
#  too, alongside backfill, despite the above), backfill detects the resulting standalone
#  specqueue plugin a few seconds after it loads and unloads it automatically - see
#  _reclaim_duplicate_specqueue(). It logs a warning and announces it once when this happens.
...
# Commands:
#   !setbots <num> | unset   (permission 3, from the original plugin) Set
#                            the target bots-per-team; 'unset' clears the
#                            admin override and returns to qlx_botsMaxBots.
#   !setskill [<0-5>]        (permission 3) Set qlx_botsSkillLevel on the
#                            fly. 0 means "use each bot's own default skill"
#                            (same meaning as qlx_botsUseBotDefaultSkills).
#                            With no argument, resets to the default
#                            (DEFAULT_BOT_SKILL_LEVEL below). Applies to
#                            bots added from now on, not bots already
#                            connected.
#   !mutebots [ON|OFF]       (permission 3) Toggle qlx_backfillMuteBots.
#                            With no argument, reports the current setting.
#                            When on (the default), chat/teamchat from bot
#                            players is dropped before it's broadcast.
...
class backfill(specqueue):
    def __init__(self):
        ...
        self.set_cvar_once("qlx_botsSkillLevel", DEFAULT_BOT_SKILL_LEVEL)
        self.set_cvar_once("qlx_botsBotOnlyGames", "1")
        self.set_cvar_once("qlx_backfillMuteBots", "1")

        # Set by handle_unload; checked by delayed callbacks and the
        # bot-only-game Timer so nothing fires against a dead instance
        # after a reload.
        self._alive = True
        ...
        self.add_hook("client_command", self.bots_handle_client_command, priority=minqlx.PRI_HIGH)
        self.add_hook("unload", self.handle_unload)

        # Minqlx bot commands
        self.add_command("setbots", self.set_bots, 3)
        self.add_command("setskill", self.cmd_setskill, 3)
        self.add_command("mutebots", self.cmd_mutebots, 3, usage="[ON|OFF]")
        ...
        try:
            if self.q_game_info[0] in SUPPORTED_GAMETYPES and new_team != "spectator" and old_team == "spectator":
                ...
                if self._queue.size() > 0 or join_locked or self.game.state in ["in_progress", "countdown"] or\
                    ...
                    @minqlx.delay(0.5)
                    def kick_a_bot():
                        if self._alive and self._queue.size() > 0:
    ...
    def bots_handle_client_command(self, player, cmd):
        # bot_nochat is an engine cvar and doesn't reliably stop QL bots
        # from using say/say_team (see the IMPORTANT note at the top of
        # this file) - so backfill drops bot chat itself, before it's
        # broadcast, whenever it's the source. Real players are never
        # touched here.
        if not self.get_cvar("qlx_backfillMuteBots", bool):
            return
        if str(player.steam_id)[0] != "9":
            return
        lowered = cmd.lower().strip()
        if lowered == "say" or lowered.startswith("say ") or \
                lowered == "say_team" or lowered.startswith("say_team "):
            return minqlx.RET_STOP_ALL
    ...
    def check_server(self):
        ...
        self._reclaim_duplicate_specqueue()
    ...
    def _reclaim_duplicate_specqueue(self):
        # backfill already loads specqueue internally by inheriting from
        # it (that's the *** IMPORTANT *** note up top). If a server
        # manager doesn't let the admin control qlx_plugins load order (or
        # an admin just doesn't know backfill already loads it), specqueue
        # can end up ALSO listed in qlx_plugins by itself. minqlx then
        # instantiates a second, standalone specqueue instance alongside
        # the one backfill embeds - double-registering its hooks/commands.
        # By the time this runs (a few seconds after backfill loads),
        # minqlx has finished its startup load pass, so any such
        # standalone copy - whether specqueue was listed before or after
        # backfill in qlx_plugins - will already show up here. Unload it,
        # leaving only backfill's own embedded copy running.
        loaded = getattr(self, "plugins", None)
        if not loaded:
            loaded = getattr(minqlx.Plugin, "_loaded_plugins", {})
        standalone = loaded.get("specqueue")
        if standalone is None or standalone is self:
            return

        minqlx.get_logger(self).warning(
            "backfill: 'specqueue' is also loaded standalone (likely listed in "
            "qlx_plugins alongside backfill, which already loads it internally - "
            "see the IMPORTANT note at the top of this file). Unloading the "
            "standalone copy so its hooks/commands aren't double-registered."
        )
        self.msg("^1backfill: ^3'specqueue' was also loaded standalone - unloading the"
                 " duplicate copy (backfill already loads it internally).")
        minqlx.console_command(
            "qlx {}unload specqueue".format(self.get_cvar("qlx_commandPrefix"))
        )
            ...
            while add:
                ...
                try:
                    ...
                    if bot_selection is None:
                        minqlx.get_logger(self).warning(
                            "backfill: ran out of available bot names; "
                            "can't add more bots this pass."
                        )
                        add = False
                        continue
    ...
    def get_bot(self):
        ...
        if len(self._avail_bots) < 1:
            # Every known persona is already in use as a connected bot.
            # Let the caller stop adding instead of crashing on pop([]).
            return None
    ...
    @minqlx.next_frame
    def _safe_kick_bot(self, client_id):
        # Re-resolves by client id and confirms it's still a bot before
        # kicking, rather than trusting a Player object captured earlier by
        # a background thread that may now point at a reused/disconnected
        # slot. Runs on the game tick, so the kick itself is frame-safe.
        try:
            p = self.player(client_id)
        except minqlx.NonexistentPlayerError:
            return
        if not p or str(p.steam_id)[0] != "9":
            return
        p.kick()

    @minqlx.next_frame
    def _safe_notify_bot_kick_pending(self):
        self.msg("^3Bot will be kicked during next round countdown to make room for the human player.")
        for spectator in self.teams()["spectator"]:
            spectator.center_print("^3Bot will be kicked during next round\n"
                ...
                while True:
                    ...
                    for bot in red:
                        if bot.score < lowest_score:
                            lowest_bot = bot
                    self._safe_kick_bot(lowest_bot.id)
            ...
            while waiting_players > 0:
                ...
                if len(blue) > 0 and len(blue) >= len(red):
                    ...
                    if self.game.state == "in_progress" and not self.in_countdown and bot.is_alive:
                        self._safe_notify_bot_kick_pending()
                        return
                    else:
                        blue.remove(bot)
                        waiting_players -= 1
    ...
    def reset_players_models(self, msg=None):
        # Only reads happen here, off the game thread; the actual .model =
        # writes are collected into `assignments` and applied together in
        # one @minqlx.next_frame pass below, re-resolving each player by id
        # right before writing instead of mutating captured Player objects.
        try:
            ...
            for pid, model in self._player_models.items():
                ...
                while count > 0:
                    ...
                    if players[count].id == pid:
                        del players[count]
                        break
                assignments.append((pid, model))
            for pl in players:
                if str(pl.steam_id)[0] == "9":
                    name = re.sub(r"\^[0-9]", "", pl.name)
                    assignments.append((pl.id, self.quake_bots[name][1]))
                elif not pl.model:
                    assignments.append((pl.id, "sarge"))

            @minqlx.next_frame
            def apply(pairs=assignments):
                for client_id, model in pairs:
                    try:
                        p = self.player(client_id)
                    except minqlx.NonexistentPlayerError:
                        continue
            ...
            minqlx.console_print("^1backfill reset_players_models Exception: {}".format(e))
    ...
    def cmd_setskill(self, player, msg, channel):
        prefix = self.get_cvar("qlx_commandPrefix")
        if len(msg) < 2:
            self.set_cvar("qlx_botsSkillLevel", DEFAULT_BOT_SKILL_LEVEL)
            player.tell("^3Bot skill level reset to the default (^1{}^3). Applies to bots added"
                        " from now on.".format(DEFAULT_BOT_SKILL_LEVEL))
            return minqlx.RET_STOP_ALL

        try:
            level = float(msg[1])
        except ValueError:
            player.tell("^3Skill level must be a number: ^1{}setskill <0-5> ^3(0 = each bot's own"
                        " default skill, no argument resets to the default)".format(prefix))
            return minqlx.RET_STOP_ALL

        if level < 0 or level > 5:
            player.tell("^3Skill level must be between ^10^3 and ^15^3.")
            return minqlx.RET_STOP_ALL

        self.set_cvar("qlx_botsSkillLevel", str(level))
        player.tell("^3Bot skill level set to ^1{}^3. Applies to bots added from now on.".format(level))
        return minqlx.RET_STOP_ALL

    def cmd_mutebots(self, player, msg, channel):
        if len(msg) < 2:
            state = "^2ON^3" if self.get_cvar("qlx_backfillMuteBots", bool) else "^1OFF^3"
            channel.reply("^3Bot chat muting is currently {}^3.".format(state))
            return minqlx.RET_STOP_ALL

        if msg[1].lower() not in ("on", "off"):
            return minqlx.RET_USAGE

        enable = msg[1].lower() == "on"
        self.set_cvar("qlx_backfillMuteBots", "1" if enable else "0")
        channel.reply("^3Bot chat muting has been turned ^{}{}^3.".format("2" if enable else "1", msg[1].upper()))
        return minqlx.RET_STOP_ALL
